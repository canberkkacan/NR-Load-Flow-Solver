...
def kacans_power_flow_solver(cdf_path):
    ...
    df = pd.DataFrame(buses_that_did_not_stuck_to_limits)
    df_volts = pd.DataFrame(bus_voltages_mean_best)
    df_angle = pd.DataFrame(bus_angles_mean_best*180/math.pi)
    new_index_labels = bus_number.flatten()
    df_volts.index = new_index_labels
    df_angle.index = new_index_labels
    df.to_excel(excel_writer="buses_stuck_to_limits.xlsx")
    df_volts.to_excel(excel_writer="bus_voltages.xlsx")
    df_angle.to_excel(excel_writer="bus_angles.xlsx")
